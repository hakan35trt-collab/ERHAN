...
export default function NotificationCenter({ currentUser }) {
  ...
  useEffect(() => {
    if (currentUser?.id) {
      ...
      // Increased interval to avoid rate limits
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // Play sound only if new unread notifications are detected
      if (newUnreadCount > prevUnreadCountRef.current) { // Simplified sound playing logic
        playNotificationSound();
      }
      
      setNotifications(userNotifications);
      setUnreadCount(newUnreadCount);
      prevUnreadCountRef.current = newUnreadCount; // Update the ref for the next comparison

    } catch (error) {
      // Handle rate limit errors gracefully
      if (error.message?.includes('429') || error.message?.includes('Rate limit')) {
        console.warn("Bildirimler rate limit nedeniyle yüklenemedi");
        return;
      }
      ...
      <DropdownMenuContent align="end" className="w-96 bg-gradient-to-b from-gray-900 to-black border-yellow-600 border-2">
        <div className="p-4 border-b border-yellow-600">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-yellow-400">🔔 Bildirimler</h3>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs opacity-75 mt-1">
                            ...
                            {new Date(notification.created_date).toLocaleTimeString('tr-TR', { 
                              hour: '2-digit', 
                              minute: '2-digit' 
                            })}
                          </div>
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          size="icon"
                          className="w-6 h-6 text-red-400 hover:bg-red-400 hover:text-white"
                        >
                          <X className="w-3 h-3" />
                        </Button>
                      </div>
